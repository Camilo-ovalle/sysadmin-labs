...
- Root domain name: `tabmonitor.local`
...
Después del reinicio, inicia sesión como `TABMONITOR\Administrator`.

### 1.4 Crear las Unidades Organizativas (OUs)

Abre **Active Directory Users and Computers** (`dsa.msc`):

1. Clic derecho sobre `tabmonitor.local` → **New** → **Organizational Unit**
...
El cliente debe resolver el dominio `tabmonitor.local`. En la configuración de red:
...
3. Selecciona **Dominio** → escribe `tabmonitor.local`
...
- Arrástralo a `tabmonitor.local > Contabilidad > Computadoras`
...
```
C:\Windows\SYSVOL\sysvol\tabmonitor.local\Policies\PolicyDefinitions\
```
...
Get-GPInheritance -Target "OU=Contabilidad,DC=tabmonitor,DC=local"
